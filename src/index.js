...
const router = createCommandRouter({
  ...
  'get-wallet': async () => {
    ...
    if (!walletId) throw new W3ActionError('MISSING_INPUT', 'wallet-id required')
  ...
  'get-wallet-action': async () => {
    ...
    if (!walletId || !actionId) throw new W3ActionError('MISSING_INPUT', 'wallet-id and action-id required')
  ...
  'get-transaction': async () => {
    ...
    if (!transactionId) throw new W3ActionError('MISSING_INPUT', 'transaction-id required')
  ...
  'get-policy': async () => {
    ...
    if (!policyId) throw new W3ActionError('MISSING_INPUT', 'policy-id required')
  ...
  'get-rule': async () => {
    ...
    if (!policyId || !ruleId) throw new W3ActionError('MISSING_INPUT', 'policy-id and rule-id required')
  ...
  'get-quorum': async () => {
    ...
    if (!quorumId) throw new W3ActionError('MISSING_INPUT', 'quorum-id required')
  ...
  'get-intent': async () => {
    ...
    if (!intentId) throw new W3ActionError('MISSING_INPUT', 'intent-id required')
  ...
  'get-vault': async () => {
    ...
    if (!vaultId) throw new W3ActionError('MISSING_INPUT', 'vault-id required')
  ...
  'get-claim': async () => {
    ...
    if (!claimId) throw new W3ActionError('MISSING_INPUT', 'claim-id required')
    setJsonOutput('result', await req('GET', `/ethereum_yield_claim/${claimId}`))
  },
  'get-sweep': async () => {
    const { req, sweepId } = setup()
    if (!sweepId) throw new W3ActionError('MISSING_INPUT', 'sweep-id required')
  ...
  'get-condition-set': async () => {
    ...
    if (!conditionSetId) throw new W3ActionError('MISSING_INPUT', 'condition-set-id required')
  ...
  'get-condition-set-item': async () => {
    ...
    if (!conditionSetId || !itemId) throw new W3ActionError('MISSING_INPUT', 'condition-set-id and item-id required')
  ...
  'delete-allowlist-entry': async () => {
    ...
    if (!entryId) throw new W3ActionError('MISSING_INPUT', 'entry-id required')
  ...
  'get-aggregation': async () => {
    ...
    if (!aggregationId) throw new W3ActionError('MISSING_INPUT', 'aggregation-id required')
...
function setup() {
  ...
  async function req(method, path, bodyObj) {
    ...
    if (!res.ok) {
      ...
      throw new W3ActionError('HTTP_ERROR', `${method} ${path} returned ${res.status}: ${text}`, { statusCode: res.status })
    }
    ...
    try {
      ...
      throw new W3ActionError('INVALID_RESPONSE', `Privy returned unparseable response: ${text.slice(0, 200)}`)
    }
  }
  ...
  function pb() {
    if (!body) throw new W3ActionError('MISSING_INPUT', 'body input is required')
